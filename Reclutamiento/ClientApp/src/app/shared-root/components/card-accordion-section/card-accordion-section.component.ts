...
import {CustomFormat} from '../../class';
...
export class CardAccordionSectionComponent implements OnInit, OnChanges {
  ...
  getHtmlRow(row: any) {
    ...
    switch (row.type) {
      case DataType.PopUp:
        result += '<a class=\"material-icons cursor-pointer\" click=\"onPopUp(' + row.value + ')\">launch</a>';
        break;
      case DataType.Link:
        result += '<a class=\"material-icons cursor-pointer\" target="_blank" href=\"' + row.value + '\">file_download</a>';
        break;
