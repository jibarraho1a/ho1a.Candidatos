...
import { ComponentConfiguration } from '../models/configuration';
import { FormControlDefinition } from '../models/form-control-definition';
...
export abstract class ComponentElement {
  ...
  getHintByControl(name: string) {
    ...
    const required = currentControl.configuration.validaciones.find(f => f.nombre === ValidationType.required && f.valor === 'true' );
    if (required) {
      result = 'Requerido';
    }
    ...
    if (pattern) {
      result += ((result.length > 0) ? ', ' : '') + 'Patrón esperado debe ser ' + pattern.valor;
    }
